...
export const CrearOrdenPage: React.FC = () => {
  ...
  const agregarFila = () => {
    setDetalles([...detalles, { id_medicamento: "", cantidad: 1, precio_unitario: 0 }]);
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-6">
      <h2 className="text-2xl font-bold flex items-center gap-2"><ShoppingCart /> Emitir Pedido</h2>
      ...
      <div className="bg-white p-6 rounded-2xl border shadow-sm space-y-4">
        ...
        <select className="w-full p-3 border rounded-xl" onChange={e => setProveedorId(e.target.value)} value={proveedorId}>
        ...
        <table className="w-full">
          ...
          <tbody>
            {detalles.map((d, i) => (
              <tr key={i}>
                <td className="p-2">
                  <select className="w-full p-2 border rounded-lg" onChange={e => manejarCambio(i, "id_medicamento", e.target.value)} value={d.id_medicamento}>
                    <option value="">Seleccione...</option>
                    {medicamentos.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}
                  </select>
                </td>
                <td className="p-2"><input type="number" className="w-20 p-2 border rounded-lg" value={d.cantidad} onChange={e => manejarCambio(i, "cantidad", e.target.value)} /></td>
                <td className="p-2"><input type="number" className="w-24 p-2 border rounded-lg" value={d.precio_unitario} onChange={e => manejarCambio(i, "precio_unitario", e.target.value)} /></td>
                <td className="p-2"><button onClick={() => setDetalles(detalles.filter((_, idx) => idx !== i))}><Trash2 className="text-error" /></button></td>
              </tr>
            ))}
          </tbody>
        </table>
        <button onClick={agregarFila} className="flex items-center gap-2 text-primary font-bold"><Plus /> Agregar ítem</button>
        <button onClick={enviarOrden} disabled={isLoading} className="w-full bg-primary text-white py-3 rounded-xl font-bold mt-4 flex justify-center hover:opacity-90 transition-opacity">
          {isLoading ? <Loader2 className="animate-spin" /> : "Confirmar y Emitir Pedido"}
        </button>
      </div>
    </div>
  );
