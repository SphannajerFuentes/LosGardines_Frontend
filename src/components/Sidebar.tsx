...
import { LayoutDashboard, Package, ShoppingCart, Activity, Users } from 'lucide-react';
...
export const Sidebar: React.FC = () => {
  ...
  const menuItems = [
    ...
    { path: '/compras', label: 'Logística e Ingresos', icon: Package, roles: ['Administrador', 'Almacenero'] },
    { path: '/catalogo', label: 'Catálogo Médico', icon: ShoppingCart, roles: ['Administrador'] },
    { path: '/operaciones', label: 'Ventas y Kardex', icon: Activity, roles: ['Administrador', 'Farmacéutico'] },
    { path: '/admin', label: 'Personal y Proveedores', icon: Users, roles: ['Administrador'] },
  ];
  ...
  return (
    <aside className="w-64 bg-surface-container-lowest border-r border-outline-variant/30 flex flex-col h-full shadow-sm z-50">
      ...
      <div className="p-6 flex items-center gap-3 border-b border-outline-variant/30">
        ...
        <span className="font-headline font-bold text-on-surface text-xl tracking-tight">Girasoles</span>
      </div>
      ...
      <div className="p-4 border-t border-outline-variant/30">
        <div className="px-4 py-3 bg-surface-container-low border border-outline-variant/20 rounded-xl">
          <p className="text-[11px] font-bold text-outline uppercase tracking-wider mb-0.5">Rol Activo</p>
