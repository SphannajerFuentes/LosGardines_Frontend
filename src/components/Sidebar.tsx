...
export const Sidebar: React.FC = () => {
  ...
  const menuItems = [
    ...
    // Usamos 'end: true' para que solo se active cuando la URL sea exactamente '/compras'
  ...
  return (
    <aside className="w-64 bg-surface-container-lowest border-r border-outline-variant/30 flex flex-col h-full z-50 overflow-hidden bg-white/50 backdrop-blur-xl">
      
      {/* Logo y Marca */}
      <div className="p-5 md:p-6 flex items-center gap-3 border-b border-outline-variant/30 flex-shrink-0">
        <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-primary-container flex items-center justify-center text-white font-bold shadow-sm">
          G
        </div>
        <span className="font-headline font-bold text-on-surface text-lg md:text-xl tracking-tight truncate">
          Girasoles
        </span>
      </div>

      {/* Navegación Dinámica */}
      <nav className="flex-1 p-3 md:p-4 space-y-1.5 overflow-y-auto custom-scrollbar">
        {menuItems
          ...
          .map((item) => {
            ...
            return (
              <NavLink
                key={item.path}
                to={item.path}
                // Si la propiedad 'exact' está definida en el menú, añadimos 'end' para evitar solapamientos
                end={item.exact}
                className={({ isActive }) =>
      ...
      <div className="p-4 border-t border-outline-variant/30 flex-shrink-0">
        <div className="px-4 py-3 bg-surface-container-low border border-outline-variant/20 rounded-xl">
          <p className="text-[10px] md:text-[11px] font-bold text-outline uppercase tracking-wider mb-0.5">
            Rol Activo
          </p>
          <p className="text-[13px] md:text-[14px] text-primary font-bold truncate">
